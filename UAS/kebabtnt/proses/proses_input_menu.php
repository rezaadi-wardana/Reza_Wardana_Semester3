...
if (!empty($_POST['input_menu_validate'])) {
    ...
    if ($cek === false) {
        ...
        if (file_exists($target_file)) {
            $message = "Maaf, File yang dimasukkan tidak ada";
            $statusUpload = 0;
        } else {
            if ($_FILES['foto']['size'] > 500000) { //500kb
                $message = "File foto yag diupload terlalu besar, Max 500Kb";
                $statusUpload = 0;

            } else {
                if ($imageType != "jpg" && $imageType != "png" && $imageType != "jpeg" && $imageType != "gif") {
                    $message = "Maaf, hanya diperboehkan gambar yang memiliki format JPG, JPEG, PNG, dan GIF";
                    $statusUpload = 0;
    ...
    window.location="../menu"
        ...
        if (mysqli_num_rows($select) > 0) {
            $message = '<script>alert("Nama menu yang dimasukkan tela ada")
        window.location="../menu"
        </script>';
        } else {
