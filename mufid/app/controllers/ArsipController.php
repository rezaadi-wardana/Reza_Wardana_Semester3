...
class ArsipController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				arsip.No_Agenda LIKE ? OR 
				arsip.Tanggal_Surat LIKE ? OR 
				arsip.Nomor_Surat LIKE ? OR 
				arsip.Perihal LIKE ? OR 
				arsip.File_Surat LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("arsip.No_Agenda", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("arsip.No_Agenda", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Arsip";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("Tanggal_Surat","Nomor_Surat","Perihal","File_Surat");
		...
		$page_title = $this->view->page_title = "Add New Arsip";
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("No_Agenda","Tanggal_Surat","Nomor_Surat","Perihal","File_Surat");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'Tanggal_Surat' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("arsip.No_Agenda", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("arsip.No_Agenda", $arr_rec_id, "in");
